package mobile.downloader.service;
...
import mobile.downloader.query.BasicDownloadInfo;
import mobile.downloader.query.DownloadQueueProvider;
import mobile.downloader.query.DownloadState;
...
public class DownloadStatusUpdater {
    ...
    public void headersReceived(final long longDownloadId,
        ...
        synchronized (downloadStatus.lock) {
            ...
            final long length = DownloadTask.getTotalLengthFromHeader(connection);
            if (length >= 0) {
            ...
            downloadStatus.setProgressMeasurer(new DownloadSpeedMeasurer(0, length));
        }
    }
